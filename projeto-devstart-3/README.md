...
**Observação: esta é uma operação irreversível. Depois de executar `eject`, você não poderá voltar atrás!**

Se você não estiver satisfeito com a ferramenta de compilação e as opções de configuração, poderá executar `eject` a qualquer momento. Este comando removerá a única dependência de compilação do seu projeto.
...
Você não precisa usar `eject`. O conjunto de recursos selecionados é adequado para implantações de pequeno e médio porte, e você não deve se sentir obrigado a usar esse recurso. No entanto, entendemos que esta ferramenta não seria útil se você não pudesse personalizá-la quando estivesse pronto.
...
### `Projeto Contador Devstart`

Criação do projeto de certificação React Devstart
...
- Incrementa o valor a cada clique no botão
...
1. Clone o repositório do github - este mesmo
2. Instale as dependências - npm
3. Execute o projeto: npm start - será aberto no localhost:3000
